	...
	if isadmin() = true then
		if trim(request("cleanall")) = "yes" then
			...
			if trim(request("changemyselect")) <> "" then
				...
				ei.Set trim(request("curdomain")), trim(request("RichEdit_Text")), trim(request("RichEdit_Html"))
			end if
		end if
	else
...
response.redirect "ok.asp?" & getGRSN() & "&gourl=" & Server.URLEncode("showadv.asp?selectdomain=" & trim(request("curdomain")) & "&isrtf=" & isrtf)
%>
