...
if trim(request("mode")) = "in" then
	...
	if Session("EnableSession") = true then
		Response.Redirect "listsesmail.asp?" & getGRSN()
	end if
end if
...
ei.LoadMailBox Session("wem"), trim(request("mode"))
...
isOutBox = false
if LCase(mb) = "out" or LCase(mb) = "sed" then
	isOutBox = true
end if
...
<link rel="stylesheet" type="text/css" href="images/listmail.css">
...
%>
	parent.f1.select_one(document.location.href, "<%=trim(request("mode")) %>");
<%
end if
%>
}

parent.f1.document.leftval.tgname.value = "<%=Server.URLEncode(mb) %>";
...
function showlog(s_url) {
	location.href = "sendloglist.asp?" + s_url;
}
...
{
	location.href = "listmail.asp?<%="mode=" & Server.URLEncode(mb) & "&sname=" & Server.URLEncode(sname) & "&sfname=" & Server.URLEncode(sfname) & addsortstr & "&" & getGRSN() %>&page=" + document.f1.page.value;
}

function emptyfolder() {
	if (confirm("<%=s_lang_0440 %>") == false)
		return ;

	location.href = "mulmail.asp?mode=cleanTrash&<%=getGRSN() %>&gourl=<%=Server.URLEncode(gourl & addsortstr) %>";
}
...
if sname = "" or sfname = "" then
	if mb = "in" then
		Response.Write s_lang_0327
	elseif mb = "out" then
		Response.Write s_lang_0332
	elseif mb = "sed" then
		Response.Write s_lang_0430
	elseif mb = "del" then
		Response.Write s_lang_0334
	else
		Response.Write server.htmlencode(mb)
	end if
else
	Response.Write server.htmlencode(sname) & "\" & server.htmlencode(sfname)
end if
%></font><%
if allnum > 0 then
...
<span style='float:left; width:3px;'>&nbsp;</span>
<span class="st_span">
<a class='wwm_btnDownload btn_gray' href="#" onclick="javascript:showlog('<%=getGRSN() %>&returl=<%=Server.URLEncode(gourl & addsortstr) %>')"><%=s_lang_0279 %></a>
</span>
<%
	end if
else
...
if page - 1 < 0 then
	...
	bottom_bar = "<a href=""listmail.asp?mode=" & Server.URLEncode(mb) & "&sname=" & Server.URLEncode(sname) & "&sfname=" & Server.URLEncode(sfname) & "&page=" & 0 & addsortstr & "&" & getGRSN() & """><img src='images/firstp.gif' border='0' align='absmiddle'></a>&nbsp;"
	bottom_bar = bottom_bar & "<a href=""listmail.asp?mode=" & Server.URLEncode(mb) & "&sname=" & Server.URLEncode(sname) & "&sfname=" & Server.URLEncode(sfname) & "&page=" & page - 1 & addsortstr & "&" & getGRSN() & """><img src='images/prep.gif' border='0' align='absmiddle'></a>&nbsp;"
	Response.Write "<a href=""listmail.asp?mode=" & Server.URLEncode(mb) & "&sname=" & Server.URLEncode(sname) & "&sfname=" & Server.URLEncode(sfname) & "&page=" & 0 & addsortstr & "&" & getGRSN() & """><img src='images/firstp.gif' border='0' align='absmiddle'></a>&nbsp;"
	Response.Write "<a href=""listmail.asp?mode=" & Server.URLEncode(mb) & "&sname=" & Server.URLEncode(sname) & "&sfname=" & Server.URLEncode(sfname) & "&page=" & page - 1 & addsortstr & "&" & getGRSN() & """><img src='images/prep.gif' border='0' align='absmiddle'></a>&nbsp;"
end if
...
if ((page+1) * pageline) => show_allnum then
	...
	bottom_bar = bottom_bar & "<a href=""listmail.asp?mode=" & Server.URLEncode(mb) & "&sname=" & Server.URLEncode(sname) & "&sfname=" & Server.URLEncode(sfname) & "&page=" & page + 1 & addsortstr & "&" & getGRSN() & """><img src='images/nextp.gif' border='0' align='absmiddle'></a>&nbsp;"
	Response.Write "<a href=""listmail.asp?mode=" & Server.URLEncode(mb) & "&sname=" & Server.URLEncode(sname) & "&sfname=" & Server.URLEncode(sfname) & "&page=" & page + 1 & addsortstr & "&" & getGRSN() & """><img src='images/nextp.gif' border='0' align='absmiddle'></a>&nbsp;"
end if
...
if page + 1 >= allpage then
	...
	bottom_bar = bottom_bar & "<a href=""listmail.asp?mode=" & Server.URLEncode(mb) & "&sname=" & Server.URLEncode(sname) & "&sfname=" & Server.URLEncode(sfname) & "&page=" & allpage - 1 & addsortstr & "&" & getGRSN() & """><img src='images/endp.gif' border='0' align='absmiddle'></a>"
	Response.Write "<a href=""listmail.asp?mode=" & Server.URLEncode(mb) & "&sname=" & Server.URLEncode(sname) & "&sfname=" & Server.URLEncode(sfname) & "&page=" & allpage - 1 & addsortstr & "&" & getGRSN() & """><img src='images/endp.gif' border='0' align='absmiddle'></a>"
end if
%></span>
    </td>
  </tr>
...
%>
	...
	<td class="st_4">
		<a href="javascript:setsort('Sender')"><%
if isOutBox = false then
	Response.Write s_lang_0147
else
	Response.Write s_lang_0441
end if
%></a><%
...
if Show_EC_Date_Style = false or (Show_EC_Date_Style = true and writeDateEC(allnum - i - 1) = true) then
	...
	show_mail_function = " onclick=""showmail('filename=" & idname & "&sname=" & Server.URLEncode(sname) & "&sfname=" & Server.URLEncode(sfname) & "&" & getGRSN() & "&gourl=" & Server.URLEncode(gourl & addsortstr) & "');"""
...
<div id="pmc_more" class="qmpanel_shadow" style="display:none; position:absolute;">
	...
	<table width="100%">
<%
if Session("EnableSession") = false and trim(request("mode")) = "in" then
%>
		<div name="mi" onclick="location.href='listsesmail.asp?set=1&<%=getGRSN() %>';" class="menu_item"><%=s_lang_0442 %></div>
		<div class="menu_item_nofun"><div style="background:#ccc; padding-top:1px; margin-top: 5px;"></div></div>
<%
end if
%>
