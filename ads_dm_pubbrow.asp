...
i = 0
allnum = ads.Count
gourl = "ads_dm_pubbrow.asp?" & getGRSN
...
function mdel()
{
	if (ischeck() == true)
	{
		...
		conv_upinfo();
		document.form1.action = "ads_dm_pubdelete.asp?<%=getGRSN() %>&mdel=1&gourl=<%=Server.URLEncode(gourl) %>";
...
function conv2ads()
{
	if (ischeck() == true)
	{
		...
		conv_upinfo();
		document.form1.action = "ads_conv.asp?cfrom=dmpads&cto=ads&<%=getGRSN() %>&gourl=<%=Server.URLEncode(gourl) %>";
		document.form1.submit();
	}
}
...
function conv2pads()
{
	if (ischeck() == true)
	{
		...
		conv_upinfo();
		document.form1.action = "ads_conv.asp?cfrom=dmpads&cto=pads&<%=getGRSN() %>&gourl=<%=Server.URLEncode(gourl) %>";
		document.form1.submit();
	}
}
<%
...
%>
		...
		location.href = "ads_dm_pubadd.asp?<%=getGRSN() %>&gourl=<%=Server.URLEncode(gourl) %>";
	else if (msi == 6)
		mdel();
<%
end if
...
<table width="100%" border="0" align="center" bgcolor="white" cellspacing="0" style="margin-top:4px;">
	...
	<span class="st_span"><span id="pm_moveto" class="menu_pop" style='width:96px; +width:99px; _width:96px;'>
	<div class='attbg'><img style='margin: 6px 0pt 0pt;' src='images/popshow.gif'></div>
	<div class='menu_pop_text'><%=s_lang_0323 %></div>
	</span></span>
...
<form action="ads_dm_pubbrow.asp" method=post id=form1 name=form1>
...
<% if isadmin() = true then %>
    ...
    <th width="9%" noWrap class="nosort"><h3><%=s_lang_0371 %></h3></th>
...
if is_domain_admin = true then
	Response.Write "<td align='center' style='border-bottom:1px solid #8CA5B5;'><a href='ads_dm_pubedit.asp?id=" & i & "&gourl=" & Server.URLEncode(gourl) & "&" & getGRSN() & "'><img src='images/edit.gif' border='0' title='" & s_lang_modify & "'></a></td>"
	Response.Write "<td align='center' style='border-bottom:1px solid #8CA5B5;'><a href='ads_dm_pubdelete.asp?id=" & Server.URLEncode(ads.nickname) & "&gourl=" & Server.URLEncode(gourl) & "&" & getGRSN() & "'><img src='images/del.gif' border='0' title='" & s_lang_del & "'></a></td>"
end if
...
<div id="pmc_moveto" class="qmpanel_shadow" style="display:none; position:absolute;">
	...
	<table width="100%">
		...
		<div name="mi" onclick="select_mto(3);" class="menu_item"><%=s_lang_0322 %></div>
	</table>
	</div>
	</div>
</div>
...
if isadmin() = true then
%>
		<div name="mi" onclick="select_mopt(4);" class="menu_item"><%=s_lang_0369 %></div>
<%
end if
...
function _save_mode(s_col, s_mode) {
	parent.f1.document.leftval.s_asp.value = "ads_dm_pubbrow.asp";
