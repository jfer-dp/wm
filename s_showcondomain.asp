...
do while i < allnum
	...
	if dmanager <> "" then
		response.write "if (isinAllUser(""" & dmanager & """) == true)"
		response.write "document.f1.username" & i & ".value = """ & dmanager & """;" & Chr(13)
	end if
...
function domainadmin_onchange(fi) {
	var nstr = ',' + fi + ',';
	if (checkstr.indexOf(nstr) == -1)
	{
		if (checkstr != "")
			nstr = fi + ',';
...
function isinAllUser(name)
{
	if (name == "")
		return true;

	var i = 0;
	for (i; i < document.f1.alluser.length; i++)
	{
		if (document.f1.alluser[i].value == name)
			return true;
	}
...
function gosub() {
	...
	if (checkstr != "")
	{
		...
		for (i = 0; i < ss.length - 1; i++)
		{
			if (ss[i].length > 0)
			{
				theObj = eval("document.f1.username" + ss[i]);

				if (theObj != null)
				{
					if (isinAllUser(theObj.value) == false)
					{
						alert("只能添加系统内用户");
						theObj.focus();
...
<input type="hidden" name="gourl" value="s_showcondomain.asp">
	<table width="97%" border="0" align="center" cellspacing="0" bgcolor="<%=MY_COLOR_3 %>">
    <tr>
	<td colspan="5" height="10" width="50%" bgcolor="#ffffff"><table><tr bgcolor="<%=MY_COLOR_2 %>"><td nowrap style="border-top:1px <%=MY_COLOR_1 %> solid; border-left:1px <%=MY_COLOR_1 %> solid; border-right:1px <%=MY_COLOR_1 %> solid; border-bottom:1px <%=MY_COLOR_1 %> solid;">
	<input type="checkbox" name="Enable_DAdminAllotSize" value="checkbox" <% if mam.Enable_DAdminAllotSize = true then response.write "checked"%>>允许域管理员分配空间&nbsp;
      ...
      <td colspan="3" align="right" bgcolor="#ffffff"><br>
        <input name="save1" type="button" value=" 保存 " onClick="javascript:gosub()" class="Bsbttn" Disabled>&nbsp;&nbsp;
...
do while i < allnum
	...
	response.write "<td style='border-bottom:1px " & MY_COLOR_1 & " solid;'><input type=""text"" name=""username" & i & """ maxlength=""64"" class=""textbox"" size='16' LANGUAGE=javascript onchange=""return domainadmin_onchange('" & i & "')"">" & Chr(13)
...
%>
    <tr>
      <td colspan="8" align="right" bgcolor="#ffffff"><br>
        <input name="save2" type="button" value=" 保存 " onClick="javascript:gosub()" class="Bsbttn" Disabled>&nbsp;&nbsp;
