...
if Request.ServerVariables("REQUEST_METHOD") = "POST" and (sname = "" or sfname = "") then
	...
	if set_mode = "1" then
		...
		exMailInfo.Load Session("wem"), trim(Request.Form("filename"))
		if bj = "1" then
			if exMailInfo.AddLabel("--star--") = false then
				is_ok = false
			else
				exMailInfo.SetStar(true)
			end if
		else
			if exMailInfo.DelLabel("--star--") = false then
				is_ok = false
			else
...
function ajax_getnp_info() {
	var post_date = "mode=<%
if Mid(gourl, 1, 8) = "listmail" then
	if sname <> "" and sfname <> "" then
		Response.Write "0"
	else
		Response.Write "1"
	end if
elseif Mid(gourl, 1, 9) = "listlabel" then
	Response.Write "3"
elseif Mid(gourl, 1, 14) = "searchlistmail" then
	Response.Write "4"
end if

Response.Write "&" & getGRSN()
%>&sname=<%=Server.URLEncode(sname) %>&sfname=<%=Server.URLEncode(sfname) %>" + parent.f1.document.leftval.sortinfo.value + "&tgname=" + parent.f1.document.leftval.tgname.value + "&fid=<%=Server.URLEncode(filename) %>";
...
%>
function killspam() {
	if (confirm("<%=s_lang_0452 %>") == false)
		return ;

	location_href("indict.asp?filename=<%=filename %>&gourl=<%=enc_gourl %>");
}

function addtrust() {
	location_href("indict.asp?filename=<%=filename %>&mode=at&gourl=<%=enc_gourl %>");
}

function movemail(tgname) {
	if (tgname == ".arc")
	{
		if (confirm("<%=s_lang_0579 %>") == false)
			return ;
	}
	...
	if (omode.indexOf("mode=" + tgname) == -1)
...
<% else %>
	if (parent.f1.document.leftval.temp.value != "|\t|")
		location_href("movemail.asp?filename=<%=filename %>&<%=getGRSN() %>&mto=" + tgname + pdeladd);
	else
		location_href("movemail.asp?filename=<%=filename %>&mto=" + tgname + "&<%=getGRSN() %>&gourl=<%=enc_gourl %>");
<% end if %>
}
...
%>
	if (parent.f1.document.leftval.temp.value != "|\t|")
		location_href("delmail.asp?filename=<%=filename %>&<%=getGRSN() %>" + pdeladd);
	else
		location_href("delmail.asp?filename=<%=filename %>&<%=getGRSN() %>&gourl=<%=enc_gourl %>");
<%
	end if
...
<%
	if Session("delProc") = 0 then
%>
	location_href("delmail.asp?realdel=1&filename=<%=filename %>&<%=getGRSN() %>&gourl=<%=enc_gourl %>");
<%
	else
%>
	if (parent.f1.document.leftval.temp.value != "|\t|")
		location_href("delmail.asp?realdel=1&filename=<%=filename %>&<%=getGRSN() %>" + pdeladd);
	else
		location_href("delmail.asp?realdel=1&filename=<%=filename %>&<%=getGRSN() %>&gourl=<%=enc_gourl %>");
<%
	end if
...
function prevnext(isnext){
	...
	if (isnext == '1' && mnext != "" && mnext != "|")
		location.href = "showmail.asp?filename=" + mnext + "&<%=getGRSN() & "&sname=" & Server.URLEncode(sname) & "&sfname=" & Server.URLEncode(sfname) %>&gourl=<%=enc_gourl %>";

	if (isnext == '0' && mprev != "" && mprev != "|")
		location.href = "showmail.asp?filename=" + mprev + "&<%=getGRSN() & "&sname=" & Server.URLEncode(sname) & "&sfname=" & Server.URLEncode(sfname) %>&gourl=<%=enc_gourl %>";
...
function need_re_getnp() {
	...
	if (ss.length > 0)
	{
		if (ss[0] == "<%=filename %>" || ss[ss.length - 1] == "<%=filename %>")
			return true;
	}
...
function writepn() {
	...
	if (mprev != "" && mprev != "|")
		writemprev = "<a href=\"javascript:prevnext('0')\"><%=s_lang_0455 %></a> ";
	else
		writemprev = "<font color='#a0a0a0'><%=s_lang_0455 %></font> ";

	if (mnext != "" && mnext != "|")
		writemnext = "<a href=\"javascript:prevnext('1')\"><%=s_lang_0456 %></a>";
	else
		writemnext = "<font color='#a0a0a0'><%=s_lang_0456 %></font>";
...
if is_inline = false then
%>
		location.href = "showmail.asp?filename=<%=filename %>&scpw=" + document.f1.sc_pw.value + "&gourl=<%=enc_gourl %>";
<%
else
%>
		location.href = "showmail.asp?filename=<%=filename %>&inlineid=<%=inlineid %>&inline=<%=trim(request("inline")) %>&scpw=" + document.f1.sc_pw.value + "&gourl=<%=enc_gourl %>";
<%
end if
%>
}
...
%>
	parent.location.href = "listlabel.asp?<%=getGRSN() %>&lbid=" + lb_id;
<%
	end if
end if
%>
...
function lbclose(tglbid, e)
{
	...
	var post_date = "setmode=2&<%=getGRSN() %>&lbid=" + tglbid + "&filename=<%=filename %>";
...
%>
	var post_date = "setmode=1&<%=getGRSN() %>&bj=" + bj + "&filename=<%=filename %>";
...
{
	var post_date = "setmode=5&<%=getGRSN() %>&bj=" + bj + "&filename=<%=filename %>";
...
{
	var post_date = "setmode=6&<%=getGRSN() %>&lbid=" + tgid + "&filename=<%=filename %>";
...
$.ajax({
	...
	success:function(data){
		...
		else
		{
			...
			if (fdid == null)
				$("#subject_td").append(get_lb_str(tgid, tgtitle, tgcolor));
		}
	},
	error:function(){
	}
...
%>
	<span class="st_span">
	<a class='wwm_btnDownload btn_gray' href="wframe.asp?mode=reply&<%=getGRSN() %>&filename=<%=filename & "&sname=" & Server.URLEncode(sname) & "&sfname=" & Server.URLEncode(sfname) & "&backurl=showmail.asp?" & enc_rqs %>"><%=s_lang_0460 %></a>
	</span>
	<span style='float:left; width:3px;'>&nbsp;</span>

	<span class="st_span">
	<a class='wwm_btnDownload btn_gray' href="wframe.asp?mode=replyall&<%=getGRSN() %>&filename=<%=filename & "&sname=" & Server.URLEncode(sname) & "&sfname=" & Server.URLEncode(sfname) & "&backurl=showmail.asp?" & enc_rqs %>"><%=s_lang_0461 %></a>
	</span>
	<span style='float:left; width:3px;'>&nbsp;</span>
...
%>
	<span class="st_span">
	<a class='wwm_btnDownload btn_gray' href="wframe.asp?mode=forward&<%=getGRSN() %>&filename=<%=filename & "&sname=" & Server.URLEncode(sname) & "&sfname=" & Server.URLEncode(sfname) & "&backurl=showmail.asp?" & enc_rqs %>"><%=s_lang_0462 %></a>
	</span>
	<span style='float:left; width:3px;'>&nbsp;</span>
<%
else
%>
	<span class="st_span">
	<a class='wwm_btnDownload btn_gray' href="wframe.asp?mode=forward&<%=getGRSN() %>&filename=<%=filename & "&sname=" & Server.URLEncode(sname) & "&sfname=" & Server.URLEncode(sfname) & "&backurl=showmail.asp?" & enc_rqs %>"><%=s_lang_0463 %></a>
	</span>
	<span style='float:left; width:3px;'>&nbsp;</span>
<%
end if
...
%>
	...
	<span class="st_span">
	<a class='wwm_btnDownload btn_gray' href="javascript:realdelthis();"><%=s_lang_0424 %></a>
	</span>
	<span style='float:left; width:3px;'>&nbsp;</span>
	...
	<span class="st_span"><span id="pm_bj" class="menu_pop"<%=s_lang_0552 %>>
	<div class='attbg'><img style='margin: 6px 0pt 0pt;' src='images/popshow.gif'></div>
...
exMailInfo.Load Session("wem"), filename
%>
...
if sname = "" or sfname = "" then
	...
	if exMailInfo.Have_Star = true then
		Response.Write "<img id='icn_star' src='images/star_yes.gif' border='0' style='cursor:pointer;' title='" & s_lang_0434 & "' onclick=""set_one_star('0');""></a>"
	else
		Response.Write "<img id='icn_star' src='images/star_no.gif' border='0' style='cursor:pointer;' title='" & s_lang_0459 & "' onclick=""set_one_star('1');""></a>"
	end if
	Response.Write "</span>"
end if
...
if slm.LoadOne(Session("wem"), filename) = true then
%>
	<tr class="head_tr"><td class="head_td">
	<span style="float:left;"><%=s_lang_0280 %><%=s_lang_mh %><a href="javascript:show_it('sendlog')"><%
if slm.Is_End = true then
	Response.Write s_lang_0273
else
	Response.Write s_lang_0274
end if
%></a></span><br>
<span id="sendlog_span" style="display:none">
<table width="100%" border="0" cellspacing="0" align="center" class="table_in">
<tr>
<td class="slog_td_1"><%=s_lang_0149 %></td>
<td class="slog_td_2"><%=s_lang_0126 %></td>
<td class="slog_td_3"><%=s_lang_0128 %></td>
</tr>
<%
i = 0
...
do while i < allnum
	...
	if i + 1 < allnum then
		bottom_line = " style='padding-left:8px; word-break:break-all; word-wrap:break-word; border-bottom:1px solid #8CA5B5;'"
	else
		bottom_line = " style='padding-left:8px; word-break:break-all; word-wrap:break-word;'"
	end if

	Response.Write "<tr>"
	Response.Write "<td bgcolor='white' height='20'" & bottom_line & ">" & server.htmlencode(name)

	if Len(cmd_str) > 2 then
		Response.Write "<br><span id='" & i & "_span' style='display:none;'><font color='#901111'>"
	...
	Response.Write "<td bgcolor='white' align='center' nowrap"
	if i + 1 < allnum then
	...
	if Len(send_date) > 4 then
		Response.Write "<td bgcolor='white' nowrap" & bottom_line & ">" & get_date_showstr(send_date) & " " & get_time_showstr(send_date) & "</td>"
	else
		Response.Write "<td bgcolor='white' nowrap" & bottom_line & ">" & get_date_showstr(Left(slm.DateStr, 12)) & " " & get_time_showstr(Left(slm.DateStr, 12)) & "</td>"
	end if
...
if ei.IsTimerSendMail = true then
	ei.GetTimerSendMailInfo y, m, d, h
	outtimer = s_lang_0492 & y & s_lang_0488 & m & s_lang_0489 & d & s_lang_0490 & h & s_lang_0491
%>
	<tr class="head_tr"><td class="head_td">
	<span style="float:left;"><%=s_lang_0493 %><%=s_lang_mh %><%
Response.Write outtimer

if sname = "" or sfname = "" then
	if isadmin() = true or Session("ReadOnlyUser") <> 1 then
%>&nbsp;&nbsp;[<a href="changetime.asp?<%=getGRSN() %>&filename=<%=filename %>&gourl=<%=enc_gourl %>"><%=s_lang_0494 %></a>]</span>
<%
	end if
end if
%>
...
if sname = "" or sfname = "" then
	if InStr(filename, ".in") then
%>
	<tr class="head_tr"><td class="head_td">
	<span style="float:left;"><%=s_lang_0498 %><%=s_lang_mh %><a href="javascript:killspam()"><%=s_lang_0499 %></a></span>
	</td></tr>
<%
	elseif InStr(filename, ".del") then
%>
	<tr class="head_tr"><td class="head_td">
	<span style="float:left;"><%=s_lang_0498 %><%=s_lang_mh %><a href="javascript:addtrust()"><%=s_lang_0500 %></a></span>
	</td></tr>
<%
	end if
end if
...
if isadmin() = true or Session("ReadOnlyUser") <> 1 then
			    	...
			    	Response.Write "<a class='att' href=""showmail.asp?filename=" & filename & "&count=" & i & "&pt=" & ei.GetAttachmentPT(show_i) & "&bd=" & Server.URLEncode(ei.GetEmlAttachmentBD(i)) & "&sname=" & Server.URLEncode(sname) & "&sfname=" & Server.URLEncode(sfname) & "&" & getGRSN() & """ target='_blank'>" & server.htmlencode(ei.GetAttachmentName(show_i)) & "</a>"
					Response.Write "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;[<a href=""showatt.asp?isdown=1&filename=" & filename & "&count=" & i & "&pt=" & pt & "&sname=" & Server.URLEncode(sname) & "&sfname=" & Server.URLEncode(sfname) & "&" & getGRSN() & """ target='_blank'>" & s_lang_0373 & "</a>]"
if isadmin() = true or Session("ReadOnlyUser") <> 1 then
					Response.Write "&nbsp;&nbsp;[<a href=""JavaScript:saveatt(" & show_i & ")"")>" & s_lang_0506 & "</a>]"
end if
...
if isadmin() = true or Session("ReadOnlyUser") <> 1 then
		    	...
		    	Response.Write "<a class='att' href=""showmail.asp?filename=" & filename & "&count=" & i & "&pt=" & ei.GetAttachmentPT(i) & "&bd=" & Server.URLEncode(ei.GetEmlAttachmentBD(i)) & "&sname=" & Server.URLEncode(sname) & "&sfname=" & Server.URLEncode(sfname) & "&" & getGRSN() & """ target='_blank'>" & server.htmlencode(ei.GetAttachmentName(i)) & "</a>"
				Response.Write "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;[<a href=""showatt.asp?isdown=1&filename=" & filename & "&count=" & i & "&pt=" & pt & "&sname=" & Server.URLEncode(sname) & "&sfname=" & Server.URLEncode(sfname) & "&" & getGRSN() & """ target='_blank'>" & s_lang_0373 & "</a>]"
if isadmin() = true or Session("ReadOnlyUser") <> 1 then
				Response.Write "&nbsp;&nbsp;[<a href=""JavaScript:saveatt(" & i & ")"")>" & s_lang_0506 & "</a>]"
end if
...
%>
	<span class="st_span">
	<a class='wwm_btnDownload btn_gray' href="delivermail.asp?filename=<%=filename %>&<%=getGRSN() & "&dsname=" & Server.URLEncode(sname) & "&dsfname=" & Server.URLEncode(sfname) %>&backurl=<%=trim(Request.QueryString) %>"><%=s_lang_0546 %></a>
	</span>
	<span style='float:left; width:3px;'>&nbsp;</span>
<%
if sname = "" or sfname = "" then
...
%>
	...
	<span class="st_span">
	<a href="javascript:realdelthis();" class="bg_href"><%=s_lang_0424 %></a>
	</span>
	<span style='float:left; width:30px;'>&nbsp;</span>
...
%>" target="_blank" class="bg_href"><%=s_lang_0373 %></a>
	...
	<span class="st_span">
	<a href="javascript:parent_href('showmail.asp?filename=<%=filename %>&gourl=<%=enc_gourl %>')" class="bg_href"><%=s_lang_0547 %></a>
	</span>
<%
end if
%>
...
<div id="pmc_moveto" class="qmpanel_shadow" style="display:none; position:absolute;">
	...
	<table width="100%">
		...
		<div name="mi" onclick="movemail('.arc');" class="menu_item"><%=s_lang_0571 %></div>
<%
dim moveto_set_max
...
<div id="pmc_bj" class="qmpanel_shadow" style="display:none; position:absolute;">
	...
	<table width="100%">
		...
		<div class="menu_item_nofun"><div style="background:#ccc; padding-top:1px; margin-top: 5px;"></div></div>
		<div name="mi" onclick="set_one_star('1');" class="menu_item"><%=s_lang_0550 %></div>
		<div name="mi" onclick="set_one_star('0');" class="menu_item"><%=s_lang_0434 %></div>
<%
set mlb = server.createobject("easymail.Labels")
mlb.Load Session("wem")
...
function create_label_str(nid, ret_title, ret_color)
	if isadmin() = true or Session("ReadOnlyUser") <> 1 then
		...
		create_label_str = "<span id=""lba_" & nid & """> <span id=""lb_" & nid & """ class=""wwm_lb_box"" style=""background:#" & ret_color & ";""><span class=""wwm_lb_text"">" & server.htmlencode(ret_title) & "</span><span id=""lbc_" & nid & """ class=""wwm_lb_close"">&nbsp;</span></span>" & Chr(13) & Chr(10) & "</span>"
	end if
end function
