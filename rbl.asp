...
<%
dim ei
set ei = server.createobject("easymail.RBL")
ei.Load

if Request.ServerVariables("REQUEST_METHOD") = "POST" then
	ip_str = trim(request("ip_tb"))
	if Len(ip_str) > 6 and trim(request("mode")) = "1" then
		isok = ei.CleanIP(ip_str)
		set ei = nothing

		if isok = true then
			Response.Redirect "ok.asp?" & getGRSN() & "&gourl=" & Server.URLEncode("rbl.asp")
		else
			Response.Redirect "err.asp?" & getGRSN() & "&gourl=" & Server.URLEncode("rbl.asp")
		end if
	end if
...
<!--
function clean_ip() {
	if (document.getElementById("ip_tb").value.length < 7)
	{
		alert("<%=s_lang_inputerr %>");
		document.getElementById("ip_tb").focus();
		return ;
	}

	document.getElementById("f1").mode.value = "1";
...
<tr><td align="left" style="background-color:white; padding-right:16px; padding-top:16px; padding-bottom:14px; border-bottom:1px #a7c5e2 solid;">
<table width="90%" border="0" align="left" cellspacing="0" bgcolor="white"><tr><td width="5%" align="left" nowrap style="padding-left:16px;">
<%=b_lang_356 & s_lang_mh %>&nbsp;<input maxlength="64" size="32" id="ip_tb" name="ip_tb" class='n_textbox'>&nbsp;
</td><td align="left" nowrap>
<a class='wwm_btnDownload btn_gray' href="javascript:clean_ip();"><%=b_lang_054 %></a>
</td></tr></table>
</td></tr>
...
<table width="90%" border="0" align="center" cellspacing="0" bgcolor="white">
  <tr valign=bottom align="center">
	<td height="10" style="color:#444444;">&nbsp;<%=b_lang_354 & s_lang_mh %></td>
	<td></td>
	<td style="color:#444444;">&nbsp;<%=b_lang_355 & s_lang_mh %></td>
  </tr>
...
<a class='wwm_btnDownload btn_blue' href="impset.asp?<%=getGRSN() %>#rbl"><< <%=s_lang_return %></a>
