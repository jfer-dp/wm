...
<%
dim ei
set ei = server.createobject("easymail.TrustUser")
'-----------------------------------------
ei.Load
...
if mode <> "" and Request.ServerVariables("REQUEST_METHOD") = "POST" then
	...
	if trim(request("mode")) <> "add" then
		set ei = nothing
		response.redirect "ok.asp?" & getGRSN() & "&gourl=trustuser.asp"
	end if
end if
...
<BODY>
<br>
<FORM ACTION="trustuser.asp" METHOD="POST" NAME="f1">
...
<table width="90%" border="0" align="center" cellspacing="0" bgcolor="<%=MY_COLOR_3 %>">
	...
	<tr bgcolor="<%=MY_COLOR_2 %>">
      <td colspan="2" height="28" style="border-bottom:1px <%=MY_COLOR_1 %> solid; border-top:1px <%=MY_COLOR_1 %> solid; border-left:1px <%=MY_COLOR_1 %> solid; border-right:1px <%=MY_COLOR_1 %> solid;"> 
        <div align="center"><font class="s" color="<%=MY_COLOR_4 %>"><b>设置信任帐号</b></font></div>
      </td>
    </tr>
<%
i = 0
  ...
  <div align="center">
    <table width="90%" border="0" align="center" cellspacing="0" bgcolor="<%=MY_COLOR_3 %>" style='border-top:1px <%=MY_COLOR_1 %> solid;'>
      ...
      <tr> 
        ...
        <td width="94%">信任帐号不受外发邮件数和抄送数的限制.<br><br>支持通配符方式. (*: 任意长度的任何内容.&nbsp;&nbsp;?: 一个字符的任何内容.)
        </td>
      </tr>
      <tr>
        <td colspan="2" height="10"> 
...
&nbsp;<a href="showsysinfo.asp?<%=getGRSN() %>#trustuser" style="text-transform: none; text-decoration: none;"><font class="s" color="<%=MY_COLOR_4 %>"><b>启动项设置</b></font>&nbsp;<img src="images\ugo.gif" border="0" align="absbottom"></a>&nbsp;
</td></tr></table>
</div>
</BODY>
</HTML>
