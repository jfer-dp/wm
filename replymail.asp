...
%>
	...
	hide_ex();
<%
if trim(request("mode")) = "replyall" then
%>
	hide_cc();
<%
end if
...
<textarea autocomplete="off" name="EasyMail_To" id="EasyMail_To" size="48" class='textbox_wwm' onfocus="setIt('EasyMail_To')" cols="40" rows="4"><%
if ei.ReplyTo = "" then
	if ei.FromName = "" then
		Response.Write ei.FromMail
	else
		Response.Write ei.FromName & " <" & ei.FromMail & ">"
	end if
else
	if ei.FromName = "" then
		Response.Write ei.ReplyTo
	else
		Response.Write ei.FromName & " <" & ei.ReplyTo & ">"
	end if
end if
%></textarea>
	</td>
...
if trim(request("mode")) = "replyall" then
	...
	do while i < allnum
		...
		if InStr(ret_email, "@") and LCase(ret_email) <> Session("mail") then
			if LCase(ret_email) = LCase(ret_name) then
				ret_name = Mid(ret_name, 1, InStr(ret_name, "@") - 1)
			end if

			if toccstr <> "" then
				toccstr = toccstr & "," & ret_name & " <" & ret_email & ">"
			else
				toccstr = ret_name & " <" & ret_email & ">"
			end if
		end if
...
%></td></tr></table>
  ...
  <tr><td width="100%">
	...
	<td align="left"><input name="EasyMail_Subject" id="EasyMail_Subject" type="text" maxlength="512" class='subject_input' value="<%=Session("addsubject") & server.htmlencode(ei.subject) %>">
	</td></tr></table>
	</td></tr>
...
intpt = ei.GetReplyMsgFromTemplet()
intpt = replace(intpt, "$QTEXT$", "")

if Session("addomail") = true then
	...
	t = replace(RemoveEndRN(t), Chr(10), Chr(10) & ">")

	if Len(intpt) > 0 then
		intpt = replace(intpt, "$QUOTES$", t)
	else
		intpt = t
	end if
else
...
<%
	...
	if Session("addomail") = true then
		...
		write_str = ""
		if Len(ei.FromName) > 0 then
			write_str = "<b>" & ei.FromName & "</b>"
		end if

		if Len(ei.FromMail) > 0 then
			if Len(write_str) > 0 and LCase(ei.FromName) <> LCase(ei.FromMail) then
				write_str = write_str & "&nbsp;<b>&lt;" & ei.FromMail & "&gt;</b>&nbsp;写道:<br>"
			else
				write_str = "<b>" & ei.FromMail & "</b>&nbsp;写道:<br>"
			end if
		else
			if Len(write_str) > 0 then
				write_str = write_str & "&nbsp;写道:<br>"
			else
				write_str = "写道:<br>"
			end if
		end if
		...
		if ei.HTML_Text <> "" then
			if Len(intpt) > 0 then
				intpt = replace(intpt, "$QUOTES$", "<BLOCKQUOTE style=""PADDING-LEFT: 5px; MARGIN-LEFT: 5px; BORDER-LEFT: #1010ff 2px solid"">" & ei.HTML_Text & "</BLOCKQUOTE>")
				Response.Write "writeRichText('richedit', RemoveScript('" & RTESafe(intpt) & "'), ""100%"", " & rtf_height & ", true, false);"
			else
				Response.Write "writeRichText('richedit', RemoveScript('" & RTESafe("<br><br>" & write_str & "<BLOCKQUOTE style=""PADDING-LEFT: 5px; MARGIN-LEFT: 5px; BORDER-LEFT: #1010ff 2px solid"">" & ei.HTML_Text & "</BLOCKQUOTE><br>") & "'), ""100%"", " & rtf_height & ", true, false);"
			end if
		else
			if LCase(ei.ContentType) = "text/html" then
				if Len(intpt) > 0 then
					intpt = replace(intpt, "$QUOTES$", "<BLOCKQUOTE style=""PADDING-LEFT: 5px; MARGIN-LEFT: 5px; BORDER-LEFT: #1010ff 2px solid"">" & ei.Text & "</BLOCKQUOTE>")
					Response.Write "writeRichText('richedit', RemoveScript('" & RTESafe(intpt) & "'), ""100%"", " & rtf_height & ", true, false);"
				else
					Response.Write "writeRichText('richedit', RemoveScript('" & RTESafe("<br><br>" & write_str & "<BLOCKQUOTE style=""PADDING-LEFT: 5px; MARGIN-LEFT: 5px; BORDER-LEFT: #1010ff 2px solid"">" & ei.Text & "</BLOCKQUOTE><br>") & "'), ""100%"", " & rtf_height & ", true, false);"
				end if
			else
				if (issign = true or isenc = true) and ei.DecryptOrVerifyStr <> "" then
					t = ei.DecryptOrVerifyStr
				...
				if Len(intpt) > 0 then
					intpt = replace(intpt, "$QUOTES$", "<BLOCKQUOTE style=""PADDING-LEFT: 5px; MARGIN-LEFT: 5px; BORDER-LEFT: #1010ff 2px solid"">" & ConvTextToHtml(t) & "</BLOCKQUOTE>")
					Response.Write "writeRichText('richedit', '" & intpt & "', ""100%"", " & rtf_height & ", true, false);"
				else
					Response.Write "writeRichText('richedit', '<br><br>" & write_str & "<BLOCKQUOTE style=""PADDING-LEFT: 5px; MARGIN-LEFT: 5px; BORDER-LEFT: #1010ff 2px solid"">" & ConvTextToHtml(t) & "</BLOCKQUOTE><br>', ""100%"", " & rtf_height & ", true, false);"
				end if
			end if
		end if
	else
		intpt = replace(intpt, "$QUOTES$", "")
		Response.Write "writeRichText('richedit', '" & ConvTextToHtml(intpt) & "', ""100%"", " & rtf_height & ", true, false);"
	end if
%>
//-->
</script>
