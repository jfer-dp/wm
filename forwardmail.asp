...
if isMSIE = true then
%>
var left_temp_val = parent.parent.f1.leftval.temp.value;
<%
else
%>
var left_temp_val = parent.parent.f1.document.leftval.temp.value;
<%
end if
%>
...
else
{
	...
	if (document.f1.IsReSend.value != "")
	{
		var s,ss;
		s = document.f1.EasyMail_OrAtt.value;
		ss = s.split("\t");

		for (i = document.f1.OrAtt.length - 1; i >= 0; i--)
		{
			if (checkOrgAtt(ss, document.f1.OrAtt.options[i].value) == false)
				document.f1.OrAtt.remove(i);
		}
	}
...
<%
if ei.ReCc <> "" then
	Response.Write "hide_cc();" & Chr(13)
end if

if ei.ReBcc <> "" then
	Response.Write "hide_bcc();" & Chr(13)
end if

if ei.AttachmentCount > 0 then
	Response.Write "hide_ex();" & Chr(13)
end if
%>
...
function getNP() {
	...
	document.f1.YH_prev.value = mprev;
	document.f1.YH_next.value = mnext;
}
...
<a href="help.asp#writemail" target="_blank"><img src="images/help.gif" border="0" title="帮助"></a></div>
   <FORM ACTION="sendmail.asp?<%=getGRSN() %>" METHOD=POST NAME="f1" target="_parent">
    <INPUT NAME="YH_sendmode" TYPE="hidden" Value="rpfw">
    <INPUT NAME="YH_filename" TYPE="hidden" Value="<%=filename %>">
    <INPUT NAME="YH_backurl" TYPE="hidden" Value="<%=trim(request("backurl")) %>">
    <INPUT NAME="YH_prev" TYPE="hidden">
    <INPUT NAME="YH_next" TYPE="hidden">
...
%>">
    ...
    <INPUT NAME="sfname" TYPE="hidden" value="<%=sfname %>">
    ...
    <INPUT NAME="IsReSend" TYPE="hidden">
...
%>
		<a class="wwm_btnDownload btn_blue" style="width: 60px;" href="#" onclick="javascript:<%
back_url = trim(request("backurl")) 
if back_url = "goback" then
	Response.Write "history.back();"
else
	Response.Write "parent.location.href='" & back_url & "'"
end if
%>">取消</a>
<%
end if
...
<textarea autocomplete="off" name="EasyMail_To" id="EasyMail_To" size="48" class='textbox_wwm' onfocus="setIt('EasyMail_To')" cols="40" rows="4"><%=ei.ReTo %></textarea>
	</td>
...
<textarea autocomplete="off" name="EasyMail_Cc" id="EasyMail_Cc" size="48" class='textbox_wwm' onfocus="setIt('EasyMail_Cc')" cols="40" rows="2" style="height:32px;"><%=ei.ReCc %></textarea>
	</td>
	<td noWrap align="left" width="4" style="padding-left:1px; padding-right:9px;">
<%
if Application("em_EnableEntAddress") = true then
...
<table cellspacing=0 cellpadding=0 width="90%" align="left" border="0"><tr>
	...
	<td noWrap align="left">
<textarea autocomplete="off" name="EasyMail_Bcc" id="EasyMail_Bcc" size="48" class='textbox_wwm' onfocus="setIt('EasyMail_Bcc')" cols="40" rows="2" style="height:32px;"><%=ei.ReBcc %></textarea>
	</td>
	<td noWrap align="left" width="4" style="padding-left:1px; padding-right:9px;">
<%
if Application("em_EnableEntAddress") = true then
...
%></td></tr></table>
  ...
  <tr><td width="100%">
	...
	<td align="left"><input name="EasyMail_Subject" id="EasyMail_Subject" type="text" maxlength="512" class='subject_input' value="<%=server.htmlencode(ei.subject) %>">
	</td></tr></table>
	</td></tr>
...
if ei.IsSaveMail = false then
	ei.Load_RP_FW Session("wem"), Session("mail"), MailName
	intpt = ei.GetForwardMsgFromTemplet()
	intpt = replace(intpt, "$QUOTES$", "")

	if (issign = true or isenc = true) and ei.DecryptOrVerifyStr <> "" then
		t = ei.DecryptOrVerifyStr
	else
		t = ei.Text
	end if

	intpt = replace(intpt, "$QTEXT$", RemoveEndRN(t))

	if Len(intpt) > 0 then
		Response.Write server.htmlencode(intpt)
	else
		Response.Write server.htmlencode(RemoveEndRN(t))
	end if
else
	if (issign = true or isenc = true) and ei.DecryptOrVerifyStr <> "" then
		t = ei.DecryptOrVerifyStr
...
if ei.IsSaveMail = false then
	...
	intpt = ei.GetForwardMsgFromTemplet()
	intpt = replace(intpt, "$QUOTES$", "")
	intpt = ConvTextToHtml(intpt)

	if ei.HTML_Text <> "" then
		if Len(intpt) > 0 then
			intpt = replace(intpt, "$QTEXT$", "<BLOCKQUOTE style=""PADDING-LEFT: 5px; MARGIN-LEFT: 5px; BORDER-LEFT: #1010ff 2px solid"">" & ei.HTML_Text & "</BLOCKQUOTE>")
			Response.Write "writeRichText('richedit', RemoveScript('" & RTESafe(intpt) & "'), ""100%"", " & rtf_height & ", true, false);"
		else
		...
		if LCase(ei.ContentType) = "text/html" then
			if Len(intpt) > 0 then
				intpt = replace(intpt, "$QTEXT$", "<BLOCKQUOTE style=""PADDING-LEFT: 5px; MARGIN-LEFT: 5px; BORDER-LEFT: #1010ff 2px solid"">" & ei.Text & "</BLOCKQUOTE>")
				Response.Write "writeRichText('richedit', RemoveScript('" & RTESafe(intpt) & "'), ""100%"", " & rtf_height & ", true, false);"
			else
			...
			if Len(intpt) > 0 then
				intpt = replace(intpt, "$QTEXT$", "<BLOCKQUOTE style=""PADDING-LEFT: 5px; MARGIN-LEFT: 5px; BORDER-LEFT: #1010ff 2px solid"">" & ConvTextToHtml(t) & "</BLOCKQUOTE>")
				Response.Write "writeRichText('richedit', '" & intpt & "', ""100%"", " & rtf_height & ", true, false);"
			else
				Response.Write "writeRichText('richedit', '" & ConvTextToHtml(t) & "', ""100%"", " & rtf_height & ", true, false);"
			end if
		end if
	end if
else
...
if ei.IsSaveMail = false then
	if userweb.EnableBackupAllSendMail = true then response.write " checked"
else
	if ei.ReSendBackup = true then response.write " checked"
end if
%>>备份
	</td></tr>
	...
	<option value="Normal"<% if ei.RePriority = 0 then response.write " selected"%>>普通邮件</option>
	<option value="Low"<% if ei.RePriority = 1 then response.write " selected"%>>慢件</option>
	<option value="High"<% if ei.RePriority = 2 then response.write " selected"%>>紧急邮件</option>
	</select>
	</td></tr>
...
%>
	...
	<td class="tr_st" align="left">
	<input name="EasyMail_BackAddress" type="text" size="30" class='n_textbox' value="<%
if ei.IsSaveMail = false then
	response.write userweb.ReMail
else
	response.write ei.ReReplyTo
end if
%>"></td></tr>
...
%>
	...
	<td class="tr_st" align="left">
	<input type="checkbox" name="EasyMail_ReadBack"<% if ei.ReReadBack = true then response.write " checked"%>>读取确认(系统内用户)
	</td></tr>
...
do while i < allnum
	Response.Write "<option value='" & i & "'>" & ei.GetAttachmentName(i) & "</option>" & Chr(13)
	i = i + 1
loop
%>
	</select> 
...
%>
	...
	<td class="tr_st" align="left">
	<input type="checkbox" name="EasyMail_SystemMail"<% if ei.ReSystemMail = true then response.write " checked"%>>是否是系统邮件
	</td></tr>
...
function ConvTextToHtml(o_str)
	o_str = replace(RemoveEndRN(o_str), "'", "&#39;")
	o_str = replace(o_str, "<", "&lt;")
	o_str = replace(o_str, ">", "&gt;")
	o_str = replace(o_str, Chr(13) & Chr(10), "<br>")
	o_str = replace(o_str, Chr(10) & Chr(13), "<br>")
	o_str = replace(o_str, Chr(13), "<br>")
	o_str = replace(o_str, Chr(10), "<br>")
	ConvTextToHtml = replace(o_str, "\", "\\")
end function
%>
