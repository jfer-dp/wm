...
dim ei
set ei = server.createobject("easymail.CollectionMail")
'-----------------------------------------
ei.Load
...
if Request.ServerVariables("REQUEST_METHOD") = "POST" then
	...
	response.redirect "ok.asp?" & getGRSN() & "&gourl=systemcollmail.asp"
end if
%>
...
function add()
{
	if (document.f1.s_collmail.value.length == 0 || document.f1.s_collmail.value.indexOf("@") == -1)
		return ;
	...
	if (haveit() == false)
	{
		...
		oOption.text = getFilterStr(document.f1.s_content.value, document.f1.s_mode.value, document.f1.s_msg.value, document.f1.s_collmail.value);
		oOption.value = document.f1.s_content.value + "\t" + document.f1.s_mode.value + "\t" + document.f1.s_msg.value + "\t" + document.f1.s_collmail.value;
<%
if isMSIE = true then
%>
...
function haveit()
{
	var tempstr = document.f1.s_content.value + "\t" + document.f1.s_mode.value + "\t" + document.f1.s_msg.value + "\t" + document.f1.s_collmail.value;
...
function getFilterStr(f_content, f_mode, f_msg, f_collmail)
{
	if (f_collmail.length == 0)
		return ;
	...
	if (f_msg.length == 0)
		retstr = retstr + "[Empty], 转发到:" + f_collmail;
	else
		retstr = retstr + f_msg + ", 转发到:" + f_collmail;
...
<div align="center"><br>
  <table width="90%" border="0" align="center" cellspacing="0" bgcolor="<%=MY_COLOR_2 %>" style="border-bottom:1px <%=MY_COLOR_1 %> solid; border-top:1px <%=MY_COLOR_1 %> solid; border-left:1px <%=MY_COLOR_1 %> solid; border-right:1px <%=MY_COLOR_1 %> solid;">
    <tr>
      ...
      <td width="32%"><a href="showsysinfo.asp?<%=getGRSN() %>#collmail">启动项设置</a></td>
      <td colspan="28"><a href="right.asp?<%=getGRSN() %>">返回</a></td>
      <td width="25%"><font class="s" color="<%=MY_COLOR_4 %>"><b>邮件采集</b></font></td>
    </tr>
  </table>
</div>
  <div align="center">
...
do while i < allnum
	ei.Get i, as_content, as_mode, as_msg, as_collmail
	Response.Write "<option value=""" & as_content & Chr(9) & as_mode & Chr(9) & server.htmlencode(as_msg) & Chr(9) & server.htmlencode(as_collmail) & """>" & server.htmlencode(getFilterStr(as_content, as_mode, as_msg, as_collmail)) & "</option>" & Chr(13)
...
<input type="input" name="s_msg" class="textbox" size="40" maxlength="100"><br>
&nbsp;&nbsp;&nbsp;转发到: <input type="input" name="s_collmail" class="textbox" size="30" maxlength="100">&nbsp;&nbsp;&nbsp;<input type="button" value=" 添加 " class="sbttn" LANGUAGE=javascript onclick="add()">
      </td></tr>
    <tr>
    <tr>
  ...
  <div align="center">
    <table width="90%" border="0" align="center" cellspacing="0" bgcolor="<%=MY_COLOR_3 %>" style='border-top:1px <%=MY_COLOR_1 %> solid;'>
      ...
      <tr> 
        ...
        <td width="94%">将符合条件的邮件收集到指定邮箱.<br>
        </td>
      </tr>
      <tr>
        <td colspan="2" height="10"> 
...
function getFilterStr(f_content, f_mode, f_msg, f_collmail)
	...
	if f_msg = "" then
		getFilterStr = getFilterStr & "[Empty], 转发到:" & f_collmail
	else
		getFilterStr = getFilterStr & f_msg & ", 转发到:" & f_collmail
	end if
end function
%>
