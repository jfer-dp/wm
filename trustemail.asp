...
dim ei
set ei = server.createobject("easymail.MailFromTrustEmail")
'-----------------------------------------
ei.Load
...
if mode <> "" and Request.ServerVariables("REQUEST_METHOD") = "POST" then
	...
	if trim(request("EnableMailFromTrustEmail")) <> "" then
		if sysinfo.EnableMailFromTrustEmail = false then
			sysinfo.EnableMailFromTrustEmail = true
			sysinfo.Save
		end if
	else
		if sysinfo.EnableMailFromTrustEmail = true then
			sysinfo.EnableMailFromTrustEmail = false
			sysinfo.Save
		end if
	end if
	...
	if trim(request("mode")) <> "add" then
		set ei = nothing
		set sysinfo = nothing
		Response.Redirect "ok.asp?" & getGRSN() & "&gourl=trustemail.asp"
	end if
end if
...
<BODY>
<br>
<FORM ACTION="trustemail.asp" METHOD="POST" NAME="f1">
<input type="hidden" name="mode">
  <table width="90%" border="0" align="center" cellspacing="0" bgcolor="<%=MY_COLOR_3 %>" style="border-bottom:1px <%=MY_COLOR_1 %> solid; border-top:1px <%=MY_COLOR_1 %> solid; border-left:1px <%=MY_COLOR_1 %> solid; border-right:1px <%=MY_COLOR_1 %> solid;">
	...
	<td width="74%"><input type="checkbox" id="EnableMailFromTrustEmail" name="EnableMailFromTrustEmail" value="checkbox"<% if sysinfo.EnableMailFromTrustEmail = true then Response.Write " checked"%>><%=s_lang_0025 %></td>
	<td width="24%" nowrap><b><%=s_lang_0023 %></b></td>
    </tr>
  </table>
<table width="90%" border="0" align="center" cellspacing="0" bgcolor="<%=MY_COLOR_3 %>">
	...
	<tr bgcolor="<%=MY_COLOR_2 %>">
      <td colspan="2" height="28" style="border-bottom:1px <%=MY_COLOR_1 %> solid; border-top:1px <%=MY_COLOR_1 %> solid; border-left:1px <%=MY_COLOR_1 %> solid; border-right:1px <%=MY_COLOR_1 %> solid;"> 
        <div align="center"><font class="s" color="<%=MY_COLOR_4 %>"><b><%=s_lang_0023 %></b></font></div>
      </td>
    </tr>
<%
i = 0
  ...
  <div align="center">
    <table width="90%" border="0" align="center" cellspacing="0" bgcolor="<%=MY_COLOR_3 %>" style='border-top:1px <%=MY_COLOR_1 %> solid;'>
      ...
      <tr> 
        ...
        <td width="94%"><%=s_lang_0024 %>.<br><br><%=s_lang_tpf %>
        </td>
      </tr>
      <tr>
        <td colspan="2" height="10"> 
