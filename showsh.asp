...
if is_inline = false then
%>
		location.href = "showsh.asp?filename=<%=filename %>&scpw=" + document.f1.sc_pw.value + "&gourl=<%=enc_gourl %>";
<%
else
%>
		location.href = "showsh.asp?filename=<%=filename %>&inlineid=<%=inlineid %>&inline=<%=trim(request("inline")) %>&scpw=" + document.f1.sc_pw.value + "&gourl=<%=enc_gourl %>";
<%
end if
%>
}
...
function setsh(fname, ispass) {
	location.href = "mulmail.asp?mto=" + fname.substr(2) + "&mode=shone" + ispass + "&gourl=<%=enc_gourl %>";
}
// -->
</script>
...
%>
	...
	<span class="st_span">
	<a class='wwm_btnDownload btn_gray' href="wframe.asp?mode=reply&<%=getGRSN() %>&filename=<%=filename & "&sname=" & Server.URLEncode(sname) & "&sfname=" & Server.URLEncode(sfname) & "&backurl=showsh.asp?" & enc_rqs %>"><%=s_lang_0460 %></a>
	</span>
	<span style='float:left; width:3px;'>&nbsp;</span>

	<span class="st_span">
	<a class='wwm_btnDownload btn_gray' href="javascript:setsh('<%=filename %>', 1)"><%=s_lang_0589 %></a>
	</span>
	<span style='float:left; width:3px;'>&nbsp;</span>

	<span class="st_span">
	<a class='wwm_btnDownload btn_gray' href="javascript:setsh('<%=filename %>', 0)"><%=s_lang_0590 %></a>
	</span>
	<span style='float:left; width:3px;'>&nbsp;</span>
		...
		if ei.AttachmentCanShow(i) = true then
			...
			if ei.GetAttachmentName(show_i) = "" then
				...
				if ei.AttachmentIsMessage(show_i) = false then
			    	...
			    	Response.Write "<a class='att' href=""showsh.asp?filename=" & filename & "&count=" & i & "&pt=" & ei.GetAttachmentPT(show_i) & "&bd=" & Server.URLEncode(ei.GetEmlAttachmentBD(i)) & "&sname=" & Server.URLEncode(sname) & "&sfname=" & Server.URLEncode(sfname) & "&" & getGRSN() & """ target='_blank'>" & server.htmlencode(ei.GetAttachmentName(show_i)) & "</a>"
					Response.Write "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;[<a href=""showatt.asp?isdown=1&filename=" & filename & "&count=" & i & "&pt=" & pt & "&sname=" & Server.URLEncode(sname) & "&sfname=" & Server.URLEncode(sfname) & "&" & getGRSN() & """ target='_blank'>" & s_lang_0373 & "</a>]"
					Response.Write "&nbsp;&nbsp;[<a href=""JavaScript:saveatt(" & show_i & ")"")>" & s_lang_0506 & "</a>]"
				end if
			end if
	...
	do while i < allnum
		...
		if ei.GetAttachmentName(i) = "" then
			...
			if ei.AttachmentIsMessage(i) = false then
		    	...
		    	Response.Write "<a class='att' href=""showsh.asp?filename=" & filename & "&count=" & i & "&pt=" & ei.GetAttachmentPT(i) & "&bd=" & Server.URLEncode(ei.GetEmlAttachmentBD(i)) & "&sname=" & Server.URLEncode(sname) & "&sfname=" & Server.URLEncode(sfname) & "&" & getGRSN() & """ target='_blank'>" & server.htmlencode(ei.GetAttachmentName(i)) & "</a>"
				Response.Write "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;[<a href=""showatt.asp?isdown=1&filename=" & filename & "&count=" & i & "&pt=" & pt & "&sname=" & Server.URLEncode(sname) & "&sfname=" & Server.URLEncode(sfname) & "&" & getGRSN() & """ target='_blank'>" & s_lang_0373 & "</a>]"
				Response.Write "&nbsp;&nbsp;[<a href=""JavaScript:saveatt(" & i & ")"")>" & s_lang_0506 & "</a>]"
			end if
		end if
...
%>" target="_blank" class="bg_href"><%=s_lang_0373 %></a>
	...
	<span class="st_span">
	<a href="javascript:parent_href('showsh.asp?filename=<%=filename %>&gourl=<%=enc_gourl %>')" class="bg_href"><%=s_lang_0547 %></a>
	</span>
<%
end if
%>
