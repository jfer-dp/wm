...
<%
dim ei
set ei = server.createobject("easymail.AttachmentExNameFilter")
'-----------------------------------------
ei.Load
...
if mode <> "" and Request.ServerVariables("REQUEST_METHOD") = "POST" then
	...
	if trim(request("mode")) <> "add" then
		set ei = nothing
		response.redirect "ok.asp?" & getGRSN() & "&gourl=exnamefilter.asp"
	end if
end if
...
<BODY>
<br>
<FORM ACTION="exnamefilter.asp" METHOD="POST" NAME="f1">
...
<table width="90%" border="0" align="center" cellspacing="0" bgcolor="<%=MY_COLOR_3 %>">
	...
	<tr bgcolor="<%=MY_COLOR_2 %>">
      <td colspan="2" height="28" style="border-bottom:1px <%=MY_COLOR_1 %> solid; border-top:1px <%=MY_COLOR_1 %> solid; border-left:1px <%=MY_COLOR_1 %> solid; border-right:1px <%=MY_COLOR_1 %> solid;"> 
        <div align="center"><font class="s" color="<%=MY_COLOR_4 %>"><b>附件名称或类型过滤</b></font></div>
      </td>
    </tr>
<%
i = 0
  ...
  <div align="center">
    <table width="90%" border="0" align="center" cellspacing="0" bgcolor="<%=MY_COLOR_3 %>" style='border-top:1px <%=MY_COLOR_1 %> solid;'>
      ...
      <tr> 
        ...
        <td width="94%">此项功能可以防止接收到可能带病毒的邮件.
        <br>在启用此功能后, 系统拒绝接收含有特定附件名称或类型的邮件.
        <br>比如: *.exe&nbsp;将可以使系统不接收所有附件类型为.exe的邮件.
        <br><br>注意: 通过WebMail发送的邮件将不受此功能的限制.
...
&nbsp;<a href="showsysinfo.asp?<%=getGRSN() %>#AttachmentExName_Filter" style="text-transform: none; text-decoration: none;"><font class="s" color="<%=MY_COLOR_4 %>"><b>启动项设置</b></font>&nbsp;<img src="images\ugo.gif" border="0" align="absbottom"></a>&nbsp;
</td></tr></table>
</div>
</BODY>
</HTML>
